test_that("the host uses the parent's installed Alder without library hints", {
  lib <- alder_cache_lib()
  ...
  # Restore that search path only in the parent; the production host must
  # propagate the installed package and dependency paths to Ark itself.
