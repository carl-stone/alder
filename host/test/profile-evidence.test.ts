...
const files = ['CPU.host.cpuprofile', 'browser.cpuprofile', 'cpu-2.Rprof', 'cpu-3.Rprof'];
...
test('profile qualification rejects missing stages, CPU profiles and mismatched execution identities', () => {
  ...
  for (const stage of ['host.engine.request', 'kernel.execute', 'analyzer.request']) {
  ...
  assert.throws(() => validateProfileEvidence(['CPU.host.cpuprofile', 'browser.cpuprofile', 'cpu-2.Rprof', 'cpu-4.Rprof'], records, samples), /analyzer CPU/);
  assert.throws(() => validateProfileEvidence(files,
