...
export interface McpControllerAdapter {
  ...
  recover(epoch: string | null, cursor: number | null): Recovery | ArtifactHandle;
...
export interface AlderMcpOptions {
  ...
  onNextCommandSequence?(next: number): void;
...
const sequence = z.number().int().min(1).max(2_147_483_647);
...
const wait = z.boolean().default(true);
...
const queryLimit = z.number().int().min(1).max(262_144).optional();

const commonEffect = { ...operationIdentity };
...
const toolSchemas: Record<string, z.ZodType> = {
  ...
  run_cell: z.object({ ...commonEffect, expectedDocumentRevision, cell: id, changes: z.array(mcpDocumentChangeSchema).max(1_000).optional(), wait }).strict(),
  run_all: z.object({ ...commonEffect, expectedDocumentRevision, changes: z.array(mcpDocumentChangeSchema).max(1_000).optional(), wait }).strict(),
  run_stale: z.object({ ...commonEffect, expectedDocumentRevision, changes: z.array(mcpDocumentChangeSchema).max(1_000).optional(), wait }).strict(),
  ...
  restart: z.object({ ...commonEffect, replay: z.boolean().default(false), expectedDocumentRevision: expectedDocumentRevision.optional(), wait }).strict(),
  ...
  materialize_output: z.object({ ...commonEffect, key: id, kernelEpoch: id, wait }).strict(),
  operation_status: z.object({ operation: id }).strict(),
  read_events: z.object({ epoch: id.nullable(), cursor: z.number().int().min(0).nullable() }).strict(),
  get_config: empty,
  ...
  set_app: z.object({ ...commonEffect, patch: jsonRecord, expectedDocumentRevision }).strict(),
  ...
  packages_install: z.object({ ...commonEffect, packages: z.array(z.string().min(1).max(256)).max(1_000), expectedDocumentRevision, kernelEpoch: id.nullable(), wait }).strict(),
  publish: z.object({ ...commonEffect, includeCode: z.boolean(), outputPath: path.optional(), expectedDocumentRevision, wait }).strict(),
...
// Selecting another R executable is the recovery path when initial runtime startup fails.
const startupRecoveryToolNames = new Set(["select_r"]);
...
export function createMcpServer(options: AlderMcpOptions): McpServer {
  ...
  server.server.oninitialized = () => {
    ...
    unsubscribe = options.controller.subscribe((event) => {
      ...
      if (event.operationId !== undefined) uris.add(`alder://operations/${encodeURIComponent(event.operationId!)}`);
  ...
  for (const name of catalogOrder) {
    ...
    registerTool(name, { description: descriptions[name]!, inputSchema: schema }, ((args: Record<string, unknown>, extra: McpRequestExtra) => {
...
async function executeTool(options: AlderMcpOptions, name: string, args: Record<string, unknown>, signal: AbortSignal, extra: McpRequestExtra, sessionSignal: AbortSignal): Promise<unknown> {
  ...
  throwIfAborted(signal);
  options.assertActive?.();
  if (name === "notebook_state") return await query({ type: "notebook" });
  ...
  if (name === "operation_status") return await query({ type: "operation", operationId: String(args.operation), clientId: options.clientId });
  if (name === "read_events") return logicalQueryResult(await query({ type: "events", epoch: args.epoch === null ? null : String(args.epoch), cursor: args.cursor === null ? null : Number(args.cursor) }));
  ...
  throwIfAborted(signal);
  // Keep this check adjacent to dispatch: lease revocation may happen while
  // the progress notification is in flight.
  options.assertActive?.();
  ...
  const onAbort = (): void => {
    ...
    cancellation ??= interruptOperation(options, operation, accepted.nextCommandSequence, sessionSignal).catch(() => undefined);
  };
  signal.addEventListener("abort", onAbort, { once: true });
  if (signal.aborted) onAbort();
  try {
    const settled = await waitForOperation(controller, operation.id, options.operationTimeoutMs ?? 120_000, signal, options.clientId);
    throwIfAborted(signal);
    options.assertActive?.();
    ...
    if (name === "shutdown" && isRecord(settled.result) && settled.result.closing === true) {
      // The SDK writes the tool response after this callback returns. Yield to
      // that response before closing the host and its MCP transport.
      setImmediate(() => {
...
async function interruptOperation(options: AlderMcpOptions, operation: OperationRecord, nextCommandSequence: number, sessionSignal: AbortSignal): Promise<void> {
  const controller = options.controller;
  if (sessionSignal.aborted) return;
...
function parseCommand(options: AlderMcpOptions, name: string, args: Record<string, unknown>): HostCommand {
  const base = {
  ...
  const transaction = (changes: readonly McpDocumentChange[]) => parseHostCommand({
    ...base,
    type: "transaction",
  ...
  switch (name) {
    case "add_cell": return transaction([{ type: "create", creationId: String(args.operationId), after: args.after === null ? null : { cellId: String(args.after) }, cellType: args.type as "code" | "markdown", body: args.body as string[], options: args.options as Record<string, string | boolean | number> }]);
    ...
    case "run_cell": return parseHostCommand({ ...base, type: "run", scope: "cell", target: { cellId: args.cell }, changes: projectOptionalChanges(args.changes), expectedDocumentRevision: args.expectedDocumentRevision });
    case "run_all": return parseHostCommand({ ...base, type: "run", scope: "all", changes: projectOptionalChanges(args.changes), expectedDocumentRevision: args.expectedDocumentRevision });
    case "run_stale": return parseHostCommand({ ...base, type: "run", scope: "stale", changes: projectOptionalChanges(args.changes), expectedDocumentRevision: args.expectedDocumentRevision });
    case "interrupt": return parseHostCommand({ ...base, type: "interrupt", ...(args.runId === undefined ? {} : { runId: args.runId }) });
    case "get_value": return parseHostCommand({ ...base, type: "inspect", name: args.name, kernelEpoch: args.kernelEpoch });
    case "set_widget": return parseHostCommand({ ...base, type: "widget", name: args.name, path: args.path, update: args.update, source: "mcp", kernelEpoch: args.kernelEpoch, expectedRevision: args.expectedRevision });
    case "save": return parseHostCommand({ ...base, type: "save", expectedDocumentRevision: args.expectedDocumentRevision });
    ...
    case "select_r": return parseHostCommand({ ...base, type: "select-r", rscript: args.rscript, persistDefault: args.persistDefault, expectedDocumentRevision: args.expectedDocumentRevision });
    case "set_runtime": return parseHostCommand({ ...base, type: "set-runtime", on_cell_change: args.on_cell_change, on_startup: args.on_startup, expectedDocumentRevision: args.expectedDocumentRevision });
    case "reload_source": return parseHostCommand({ ...base, type: "reload-source", expectedDocumentRevision: args.expectedDocumentRevision, expectedDiskDigest: args.expectedDiskDigest, expectedDiskVersion: args.expectedDiskVersion });
    case "shutdown": return parseHostCommand({ ...base, type: "shutdown", expectedDocumentRevision: args.expectedDocumentRevision, expectedClientIds: args.expectedClientIds });
    case "restart": return parseHostCommand({ ...base, type: "restart", replay: args.replay, ...(args.expectedDocumentRevision === undefined ? {} : { expectedDocumentRevision: args.expectedDocumentRevision }) });
    case "format": return parseHostCommand({ ...base, type: "format", cellIds: args.cellIds, expectedRevisions: args.expectedRevisions, expectedDocumentRevision: args.expectedDocumentRevision });
    case "save_as": return parseHostCommand({ ...base, type: "save-as", path: args.path, expectedDestination: args.expectedDestination, expectedDocumentRevision: args.expectedDocumentRevision });
    case "table_page": return parseHostCommand({ ...base, type: "table-page", handle: args.handle, offset: args.offset, limit: args.limit, sortBy: args.sortBy, sortDescending: args.sortDescending, filter: args.filter, kernelEpoch: args.kernelEpoch });
    case "materialize_output": return parseHostCommand({ ...base, type: "lazy-output", key: args.key, kernelEpoch: args.kernelEpoch });
    case "set_config": return parseHostCommand({ ...base, type: "set-config", patch: args.patch, expectedSidecarVersion: args.expectedSidecarVersion, expectedDocumentRevision: args.expectedDocumentRevision });
    case "set_layout": return parseHostCommand({ ...base, type: "set-layout", layout: args.layout, expectedSidecarVersion: args.expectedSidecarVersion, expectedDocumentRevision: args.expectedDocumentRevision });
    case "set_app": return parseHostCommand({ ...base, type: "set-app", patch: args.patch, expectedDocumentRevision: args.expectedDocumentRevision });
    case "packages_declare": return parseHostCommand({ ...base, type: "packages-declare", packages: args.packages, expectedSidecarVersion: args.expectedSidecarVersion, expectedDocumentRevision: args.expectedDocumentRevision });
    case "packages_install": return parseHostCommand({ ...base, type: "packages-install", packages: args.packages, expectedDocumentRevision: args.expectedDocumentRevision, kernelEpoch: args.kernelEpoch });
    case "publish": return parseHostCommand({ ...base, type: "publish", includeCode: args.includeCode, outputPath: args.outputPath, expectedDocumentRevision: args.expectedDocumentRevision });
    case "upload_file": return parseHostCommand({ ...base, type: "upload", name: args.name, path: args.path, files: args.files, kernelEpoch: args.kernelEpoch });
...
async function waitForOperation(controller: McpControllerAdapter, operationId: string, timeoutMs: number, signal: AbortSignal | undefined, clientId: string): Promise<OperationRecord> {
  const current = controller.operation?.(operationId, clientId);
  if (current !== undefined && current.status !== "accepted" && current.status !== "running") return current;
  if (controller.awaitOperation === undefined) throw codedError("operation_unavailable", "operation waiting is unavailable");
  const timeout = new AbortController();
  const timer = setTimeout(() => timeout.abort(codedError("mcp_timeout", "MCP action timed out")), timeoutMs);
  timer.unref?.();
  const combined = signal === undefined ? { signal: timeout.signal, dispose: () => undefined } : combineSignals(signal, timeout.signal);
  try { return await controller.awaitOperation(operationId, clientId, combined.signal); }
  catch (error) {
...
function settledValue(accepted: CommandAdmission, operation: OperationRecord): Record<string, unknown> {
  return { operation, result: { admission: accepted, value: operation.result }, ...(operation.error === undefined ? {} : { error: operation.error }) };
}

function receipt(accepted: CommandAdmission): Record<string, unknown> {
  return { operation: accepted.operation, result: { admission: accepted }, ...(accepted.error === undefined ? {} : { error: accepted.error }) };
}

function requireAccepted(admission: CommandAdmission): OperationRecord {
  if (admission.accepted && admission.operation !== null) return admission.operation as unknown as OperationRecord;
  const failure = codedError(admission.error?.code ?? "command_rejected", admission.error?.message ?? "command rejected") as Error & { envelope?: Record<string, unknown> };
  failure.envelope = { result: { admission }, ...(admission.error === undefined ? {} : { error: admission.error }) };
  throw failure;
}

function operationFailure(name: string, accepted: CommandAdmission, operation: OperationRecord): Error & { envelope: Record<string, unknown> } {
  const failure = codedError(operation.error?.code ?? `${name}_failed`, operation.error?.message ?? `${name} operation failed`) as unknown as Error & { envelope: Record<string, unknown> };
...
function normalizeEnvelope(value: unknown, snapshot: HostSnapshot): Record<string, unknown> {
  ...
  if (isRecord(value) && ("operation" in value || "error" in value || "result" in value)) return { ...base, ...value };
...
function resources(options: AlderMcpOptions, requireInitialized: (extra?: McpRequestExtra) => Promise<void>): Array<[string, string | ResourceTemplate, string, string, ResourceReader]> {
  const read: ResourceReader = async (uri, variables, extra) => {
    ...
    if (variables?.operation !== undefined) {
      const raw = variables.operation; const operationId = decodeURIComponent(Array.isArray(raw) ? raw[0]! : raw);
  ...
  const opTemplate = new ResourceTemplate("alder://operations/{operation}", { list: undefined });
