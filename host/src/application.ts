...
async function startNotebookHost(
  ...
  const retainedSaveAsRecovery: RecoveryWriter[] = [];
  ...
  const sameContent = (left: { digest?: string | null; state: string }, right: { digest?: string | null; state: string }): boolean => left.state === right.state && left.digest === right.digest;
    ...
    const sourceCommit: SourceCommitHandler = async (request, context) => {
      ...
      if (request.kind === "save") {
        ...
        try {
          ...
          if (saveAsIdentityPending && result.durabilityWarning === undefined) {
            try { await recovery!.adoptRecoveryId(recovery!.recoveryId); saveAsIdentityPending = false; }
            catch (error) { identityWarning = "Saved bytes remain visible, but recovery identity could not be confirmed; choose Save again: " + errorMessage(error); }
          }
          const durabilityWarning = [result.durabilityWarning, identityWarning].filter(Boolean).join(" ") || undefined;
          let clearError: unknown = null;
          let cleared = false;
          if (durabilityWarning === undefined && recoveryPending && recoveryFingerprint !== undefined) {
            try {
          ...
          if (durabilityWarning === undefined && clearError === null) {
            for (const retained of retainedSaveAsRecovery) {
              try { await retained.retire(); }
              catch (error) { clearError = error; }
            }
            if (clearError === null && untitledRecoveryDescriptor !== undefined) {
              try { await retireUntitledRecoveryDescriptor(untitledRecoveryDescriptor); untitledRecoveryDescriptor = undefined; }
              catch (error) { clearError = error; }
            }
            if (clearError === null) {
          ...
          if (durabilityWarning !== undefined) controller?.recordActionError(durabilityWarning, "save_durability_uncertain");
          else controller?.clearActionError("save_durability_uncertain");
          const saveResult = { ...result, ...(durabilityWarning === undefined ? {} : { durabilityWarning }) };
        ...
        try {
          ...
          await preparedOwner.commit(async () => {
            return async () => {
              ...
              if (durabilityWarning === undefined) {
                try { await oldRecovery.retire(); }
                catch (error) { durabilityWarning = "Saved destination, but old recovery cleanup failed; choose Save again: " + errorMessage(error); }
                if (durabilityWarning === undefined) {
                  for (const retained of retainedSaveAsRecovery) {
                    try { await retained.retire(); }
                    catch (error) { durabilityWarning = "Saved destination, but earlier recovery cleanup failed; choose Save again: " + errorMessage(error); }
                  }
                }
                if (durabilityWarning === undefined && untitledRecoveryDescriptor !== undefined) {
                  try { await retireUntitledRecoveryDescriptor(untitledRecoveryDescriptor); untitledRecoveryDescriptor = undefined; }
                  catch (error) { durabilityWarning = "Saved destination, but untitled recovery cleanup failed; choose Save again: " + errorMessage(error); }
                }
                if (durabilityWarning === undefined) {
                  for (const retained of retainedSaveAsRecovery) ownership.releaseRetained(retained.key);
                  retainedSaveAsRecovery.length = 0;
              ...
              retainOldClaim = durabilityWarning !== undefined;
          ...
          if (durabilityWarning !== undefined) savePublication = { ...savePublication!, result: { ...savePublication!.result, durabilityWarning } };
      ...
      if (request.kind === "reload-source" || request.kind === "watcher") {
        ...
        try {
          ...
          const sidecarObservationsChanged = !sameObservation(context.sidecars.config, currentSidecars.config)
          ...
          const sidecarsChanged = !sameContent(context.sidecars.config, currentSidecars.config)
            || !sameContent(context.sidecars.layout, currentSidecars.layout)
            || !sameContent(context.sidecars.packages, currentSidecars.packages);
          ...
          if (request.kind === "watcher" && !sourceChanged && !sidecarsChanged) {
            ...
            await observed.store.close();
            publishSource(context, { document: context.document, path: context.path, layout: context.layout,
              disk: current, sidecars: currentSidecars, dirty: context.dirty, advanceRevision: false });
