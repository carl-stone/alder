...
**Correction underway:** clean `1cf7aad` adds a post-conflict assertion that a
second edit is still renderer-local before Save. The packaged journey passes twice
for atomic replacement, Git reset, rename and delete, with recovery copies reopening
in a fresh packaged process. Focused review passed, but full acceptance exposed an
intermittent `ECONNRESET` in the earlier host-exit Quit regression after normal
release. The primary implementer is determining whether this is a product transport
gap or test socket race, then correcting the right boundary. A green retry alone is
not acceptance. The hidden path exercises renderer Save As, not the native chooser.
...
| Native Mac reliability qualification | Implementing | Primary implementer | Resolve intermittent host-exit Quit regression from full acceptance, then review a clean candidate and repeat the complete gate |
