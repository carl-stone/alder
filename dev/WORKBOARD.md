...
**Candidate ready:** clean `6f38788` corrects the zero-window Quit lifecycle after
the notebook host idles out; it follows `5c80216`, which keeps the Mac app resident
after its last window closes and adds a packaged path-forwarding/reopen journey.
Focused checks and both packaged window journeys pass. Independent correction review
is underway before full signed-app acceptance. The headless reopen journey uses
forced teardown and does not qualify native Quit. Finder/Open, Save As replacement,
...
| Native Mac reliability qualification | Review | Lead and independent reviewers | Review `6f38788`; run complete signed-app acceptance on the accepted exact commit, then continue native Mac qualification |
