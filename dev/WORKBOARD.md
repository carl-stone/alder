...
**Candidate ready:** clean `1cf7aad` adds a post-conflict assertion that a second
edit is still renderer-local before Save. The packaged journey now passes twice for
atomic replacement, Git reset, rename and delete: each preserves an acknowledged
unsaved edit, the local draft and independent disk contents; Save As recovery copies
reopen in a fresh packaged process. The correction is in focused review. The hidden
path exercises the renderer Save As command, not the native chooser.
...
| Native Mac reliability qualification | Review | Lead and independent reviewers | Review clean `1cf7aad`; accept and integrate if the packaged journey proves external-path safety, then assign the next bounded Mac reliability slice |
