...
**Correction underway:** clean `5c80216` keeps the Mac app resident after its last
window closes and adds a separate packaged path-forwarding/reopen journey. Focused
review found that Quit can retry release of an already closed session after its host
idles out, preventing exit. The primary implementer is correcting that lifecycle
boundary and checking zero-window Quit. The headless reopen journey passes but uses
forced teardown; it does not qualify native Quit. Finder/Open, Save As replacement,
dirty-close sheets and Dock activation still require a supervised foreground pass.
...
| Native Mac reliability qualification | Implementing | Primary implementer | Correct the zero-window Quit lifecycle bug; then review a clean candidate and qualify native Mac journeys without lost work or stranded processes |
