...
**Candidate ready:** clean `2f1516d` deterministically checks both
`ECONNREFUSED` and `ECONNRESET` after acknowledged normal release, while active
lease failures still block Quit. Its parent `8b1ac98` extends the narrow host-gone
condition to `ECONNRESET` after an earlier host-exit regression failed twice during
full acceptance. Focused and broad host checks pass; final test review is underway
before complete signed-app acceptance.
...
| Native Mac reliability qualification | Review | Lead and independent reviewers | Review clean `2f1516d`; repeat complete signed-app gate and external-path journey, then integrate if passed |
